...
const legacyTypeLabels: Partial<Record<ItineraryItemType, string>> = {
  flight: 'Flight (legacy item)', hotel: 'Hotel (legacy item)', transport: 'Transport (legacy item)',
};
...
const ItineraryItemModal: React.FC<ItineraryItemModalProps> = ({
  ...
  const title = mode === 'add' ? 'Add Plan item' : 'Edit Plan item';
  const context = dayNumber ? `Day ${dayNumber}${timeOfDay ? ` · ${timeOfDay[0].toUpperCase()}${timeOfDay.slice(1)}` : ''}` : undefined;
  const typeOptions = legacyTypeLabels[form.type]
    ? [{ value: form.type, label: legacyTypeLabels[form.type] as string }, ...itineraryItemTypes]
    : itineraryItemTypes;
    ...
    >
      <form id="itinerary-item-form" onSubmit={onSubmit} className="space-y-5">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div data-itinerary-field="type"><Select label="Type" value={form.type} onChange={(value) => onChange('type', value)} options={typeOptions} /></div>
        </div>
